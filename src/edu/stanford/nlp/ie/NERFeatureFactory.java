...
//    System.err.println(doFE+"\t"+domain);
  ...
  private final Collection<String> getCNWords(String rep,String suffix){
        List<String> ws = Generics.newLinkedList();
  ...
  protected Collection<String> featuresC(PaddedList<IN> cInfo, int loc) {
    ...
    if (flags.useInternal && flags.useExternal ) {
      ...
      if (flags.useLemmas) {
        ...
        //WCN
        if (flags.useWcn) {
            String wcn = c.getString(CoreAnnotations.WCNAnnotaion.class);
            if (! "".equals(wcn)) {
                featuresC.addAll(getCNWords(wcn,"-WCN"));
            }
        }
        if(flags.useWCNTrigram){
            Collection<String> cwcn = getCNWords(c.getString(CoreAnnotations.WCNAnnotaion.class),"");
            Collection<String> pwcn = getCNWords(p.getString(CoreAnnotations.WCNAnnotaion.class),"");
            Collection<String> nwcn = getCNWords(n.getString(CoreAnnotations.WCNAnnotaion.class),"");
            for(String cw:cwcn)
                for(String pw:pwcn)
                    for(String nw:nwcn){
                        featuresC.add(pw+"#"+cw+"#"+nw+"-WTRI");
                    }
        }
